...
-- Verifica se é client
create or replace function is_client()
returns boolean as $$
  select role = 'client' from profiles where id = auth.uid();
$$ language sql security definer stable;

-- Verifica se cliente tem acesso a um depositante
...
-- =====================================================================
-- STORAGE POLICIES (executar via Supabase Dashboard ou Storage SQL)
-- =====================================================================
-- Buckets a criar manualmente:
--   box-photos       (private)
--   item-photos      (private)
--   invoice-xmls     (private)
--
-- Policies sugeridas para cada bucket:
--   - SELECT: usuário autenticado com vínculo (cliente do return ou operator/manager)
--   - INSERT: operator (para fotos e XMLs originais), client (para XMLs de devolução)
--   - DELETE: manager apenas
-- (Configurar via interface do Supabase ou SQL após criação dos buckets.)

-- =====================================================================
-- JOBS AGENDADOS (pg_cron)
-- =====================================================================

-- Job 1: Auto-decisão após 72h
...
-- Job 2: Marcador para envio de aviso 24h antes da auto-decisão.
-- O envio do e-mail em si será feito por Edge Function que lê esta query.
-- Esta função apenas marca quais devoluções precisam receber aviso.
...
-- Job 3: Limpeza de fotos com mais de 1 ano após processed_at
...
-- A remoção física dos arquivos será feita por Edge Function
-- que consome esta lista e chama Supabase Storage para deletar.

-- =====================================================================
-- DADOS INICIAIS (seed)
-- =====================================================================

-- O primeiro manager precisa ser criado manualmente:
-- 1. Crie o usuário via Supabase Auth (email + senha temporária ou magic link)
-- 2. Pegue o id em auth.users
-- 3. Execute:
-- insert into profiles (id, role, full_name) values
--   ('<uuid-do-auth-user>', 'manager', 'Nome do Admin');

-- =====================================================================
-- GRANTS (Supabase faz automaticamente, mas vale revisar)
-- =====================================================================
